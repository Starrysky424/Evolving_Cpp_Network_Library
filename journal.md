...
1. **完成网络库性能测试**
   ...
   * 完成并发连接数测试，观察到连接数增加后，QPS 在高并发阶段逐渐下降，同时 P99 延迟明显上升。
   * 完成不同消息大小测试，确认消息越大，QPS 越低，但网络吞吐量越高。
   * 解决小包场景下多次 `send()` 导致的异常延迟问题。
   * 解决 Buffer 已读取数据长期占用内存的问题，避免高数据量测试触发 OOM。
   * 最终 16 KB 消息测试成功完成 100,000 次请求，0 错误，QPS 约 32.7k，吞吐量约 510 MB/s。

2. **分析项目后续开发方向**

   * 对照参考网络库，确定下一阶段重点从性能测试转向**功能正确性测试**。
   * 测试顺序确定为：`Buffer` → `Decoder` → `ConnectionManager`。

3. **开始单元测试**

   * 根据项目当前实际代码编写 `Buffer` 测试。
   * 覆盖初始化、数据写入、数据读取、`peek`、部分消费、继续追加数据、超量 `fetch` 和空数据等情况。
   * 配置 `test/CMakeLists.txt`，将测试程序链接到 `network_lib`。
   * 在根目录 CMake 中加入 `test` 子目录。
   * 成功编译并运行 `test_buffer`，测试通过。

### 当前项目状态

目前已经完成：
**核心网络功能 → 性能测试 → 性能问题修复 → 基础单元测试**

下一步继续完成 `Decoder` 单元测试，重点验证**完整包、半包和粘包**，再继续测试 `ConnectionManager`。
